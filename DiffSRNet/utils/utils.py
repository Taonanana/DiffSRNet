...
#这是一个用于测量图像质量的类，包含了几个常用的指标，如PSNR、SSIM和LPIPS。
# 其中，PSNR是峰值信噪比，用于衡量重构图像与原始图像之间的差异；
# SSIM是结构相似性指标，衡量两张图像在结构和信息方面的相似度；
# LPIPS是感知距离指标，用于测量人类视觉系统对两张图像之间的感知差异
class Measure:
    ...
    def measure(self, imgA, imgB, img_lr, sr_scale):
        ...
        if isinstance(imgA, torch.Tensor):
            ...
            # img_lr_up = np.round((img_lr_up.cpu().numpy() + 1) * 127.5).clip(min=0, max=255).astype(np.uint8)
        imgA = imgA.transpose(1, 2, 0)
        imgA_lr = imresize(imgA, 1 / sr_scale)
        imgB = imgB.transpose(1, 2, 0)
        img_lr = img_lr.transpose(1, 2, 0)
        # img_lr_up = img_lr_up.transpose(1, 2, 0)
        psnr = self.psnr(imgA, imgB)
        ssim = self.ssim(imgA, imgB)
        lpips = self.lpips(imgA, imgB)
        lr_psnr = self.psnr(imgA_lr, img_lr)
        mse_loss = self.mse(imgA, imgB)
        # mse_up_sr = self.mse(img_lr_up, imgA)
        # res = {'psnr': psnr, 'ssim': ssim, 'lpips': lpips, 'lr_psnr': lr_psnr, 'mse':mse_loss, 'mse_up_sr': mse_up_sr}
        res = {'psnr': psnr, 'ssim': ssim, 'lpips': lpips, 'lr_psnr': lr_psnr, 'mse':mse_loss}
    ...
    def mse(self, imgA, imgB):
        imgA = torch.tensor(imgA).float()
        imgB = torch.tensor(imgB).float()
        # 计算均方误差
        mse = torch.mean((imgA - imgB)**2)
        return mse
