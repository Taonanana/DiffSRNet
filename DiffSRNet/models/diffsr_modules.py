...
from models.SE_ASPP import SE_ASPP
...
class Unet(nn.Module):
    def __init__(self, dim, out_dim=None, dim_mults=(1, 2, 4, 8), cond_dim=32):
        ...
        self.aspp = SE_ASPP(mid_dim, mid_dim)
    ...
    def forward(self, x, time, cond, img_lr_up):
        ...
        for i, (resnet, resnet2, downsample) in enumerate(self.downs):
            print('before_res', i, x.shape)
            x = resnet(x, t)
            print('res1', i, x.shape)
            x = resnet2(x, t)
            print('res2', i, x.shape)
            if i == 0:
                x = x + cond
                if hparams['res'] and hparams['up_input']:
            ...
            h.append(x)
            # x1 = x
            print('before_downsample', i, x.shape)
            x = downsample(x)
            # x2 = x
            # if i == 2 : print(x1 - x2)
            print('after_downsample', i, x.shape)
        ...
        for ECAAttention, resnet, resnet2, upsample in self.ups:
            # 引入ECA模块
            # print('before_eca', x.shape)
            x = ECAAttention(x)
            # print('after_eca', x.shape)
            x = torch.cat((x, h.pop()), dim=1)
            # print('eca_cat', x.shape)
            x = resnet(x, t)
            # print('res1', x.shape)
            x = resnet2(x, t)
            # print('res2', x.shape)
