...
from models.DMP_Solver import DPM_Solver, NoiseScheduleVP, model_wrapper
...
class Trainer:
    def __init__(self):
        ...
        self.metric_keys = ['psnr', 'ssim', 'lpips', 'lr_psnr', 'mse']
    ...
    def train(self):
        ...
        self.global_step = training_step = load_checkpoint(model, optimizer, hparams['work_dir'])#rrdb checkpoints 109050
    ...
    def validate(self, training_step):
        ...
        for batch_idx, batch in pbar:
            ...
            # self.gen_dir = f"{hparams['work_dir']}/results_{self.global_step}"
            ...
            #     os.makedirs(f"{gen_dir}/valid_intermediate/{item_name}", exist_ok=True)
            #     Image.fromarray(self.tensor2img(img_hr)[0]).save(f"{gen_dir}/valid_intermediate/{item_name}/H.png")
            #     Image.fromarray(self.tensor2img(img_lr)[0]).save(f"{gen_dir}/valid_intermediate/{item_name}/LR.png")
            #     Image.fromarray(self.tensor2img(img_lr_up)[0]).save(f"{gen_dir}/valid_intermediate/{item_name}/LR_UP.png")
            ...
            #                 f"{gen_dir}/valid_intermediate/{item_name}/noise1_{i:03d}.png")
            #             Image.fromarray(self.tensor2img(m)[0]).save(
            #                 f"{gen_dir}/valid_intermediate/{item_name}/noise_{i:03d}.png")
            #             Image.fromarray(self.tensor2img(x_recon)[0]).save(
            #                 f"{gen_dir}/valid_intermediate/{item_name}/{i:03d}.png")
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in pbar:
                ...
                if hparams['save_intermediate']:
                    ...
                    Image.fromarray(self.tensor2img(img_hr)[0]).save(f"{gen_dir}/intermediate/{item_name}/H.png")
                    Image.fromarray(self.tensor2img(img_lr)[0]).save(f"{gen_dir}/intermediate/{item_name}/LR.png")
                    Image.fromarray(self.tensor2img(img_lr_up)[0]).save(f"{gen_dir}/intermediate/{item_name}/LR_UP.png")
                    ...
                    for i, (m, x_recon) in enumerate(tqdm(imgs)):
                        if i % (hparams['timesteps'] // 20) == 0 or i == hparams['timesteps'] - 1:
                            ...
                            #         method="multistep",
                            #     )
                                # Image.fromarray(self.tensor2img(x_sample)[0]).save(
                                # f"{gen_dir}/intermediate/{item_name}/x_sample_{i:03d}.png")
                            Image.fromarray(self.tensor2img(x_t)[0]).save(
                                f"{gen_dir}/intermediate/{item_name}/x_t_{i:03d}.png")
                            Image.fromarray(self.tensor2img(m)[0]).save(
